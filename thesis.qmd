...
When exchangeability is violated between the test and calibration set, achieving reliable coverage can be even more challenging. Methods such as adaptive conformal inference [@gibbs] use an adaptive time-varying parameter that is continuously re-estimated to maintain coverage. This method requires online feedback. However, in our application labels are obtained through manual annotation, which occurs offline with significant delay. 

Other approaches specifically target covariate shift, such as @Tibshirani, which uses weighting based on the likelihood ratio between test and calibration distributions. However, this requires estimating the density ratio $p_{\text{test}} (x) / p_{\text{train}(x)}$, which can be challenging to estimate reliably in high-dimensional imagery.
