...
We then compute the ensemble-averaged calibrated probability for class $y$ at pixel $x$, $$\hat{p}_y(x)= \frac{1}{K} \sum_{k=1}^K p^{(k)}_y(x; T_k).$$ The ensemble prediction is thus the argmax aggregated probability. Each model’s individual probability output is also retained for computing the epistemic uncertainty proxy, member disagreement, described in the next section.
