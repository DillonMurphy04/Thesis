...
As previously mentioned, spatial robustness is evaluated by grouping the 2022 points into ten spatial blocks based on principal-component ordering of the coordinates and then computing coverage within each block. @tbl-spatial-diagnostics reports the standard deviation of the block-wise coverage and the correlation between the block-wise mean ensemble variance and coverage. As expected, vanilla CP displays the highest block-wise standard deviation in coverage (SD=0.071) as well as a fairly strong negative correlation between mean variance and coverage (r= -0.61, p=0.06). Linear normalization reduces spatial variability and weakens the correlation compared to vanilla, while the nonparametric method yields the least variable blocks (SD=0.041) and the most weakly correlated coverage (r=0.041, p>0.5). 
