...
Finally, @fig-clipped-raster, @fig-spatial-sets, and @fig-changes show the actual raster and overlaid predictions. @fig-clipped-raster shows the 2022 raster as well as the ground truth points. As seen in the figure, the majority of eelgrass points are centered in the middle of the estuary imagery.
...
@fig-changes provides a simplified view of @fig-spatial-sets by simply visualizing the sets that changed from vanilla to each variance-normalized method. The uncertain regions align between the two methods, as many of the two-set class changes occur in the same relative area. Interestingly, the non-parametric method creates more empty set classes from vanilla compared to linear, as the expected score over-normalizes these raw score values.
